...
export async function callGemini(prompt, apiKey, options = {}) {
  const { file, temperature = 0.2, maxTokens = 65536 } = options;
  ...
  const res = await fetch(`${GEMINI_API_URL}?key=${geminiApiKey}`, {
    ...
    body: JSON.stringify({
      ...
      generationConfig: {
        ...
        // Disable thinking for predictable output length — analysis prompt already guides reasoning
  ...
  // If response was truncated (MAX_TOKENS), append warning
  if (finishReason === 'MAX_TOKENS' && result.length > 0) {
    console.warn(`[Gemini] Response truncated at ${result.length} chars (MAX_TOKENS)`);
  }
