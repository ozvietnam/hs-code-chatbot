...
FORMAT BẮT BUỘC (viết GỌN, ĐÚNG thứ tự, KHÔNG bỏ section nào):
📋 HỒ SƠ: Tên VN/EN, cấu tạo, chức năng, công dụng (3-5 dòng)
📦 PHÂN LOẠI: Chức năng chính, trạng thái, GIR áp dụng (2-3 dòng)
📊 SO SÁNH (bảng ≥3 mã, tối đa 6 mã): | Mã HS | Mô tả | Phù hợp nếu | Căn cứ |
💰 THUẾ (bảng từ fact_layer, CHỈ cho top 3 mã): | Mã HS | MFN | ACFTA | VAT | BVMT |
📝 MÔ TẢ ECUS: gợi ý mô tả khai ECUS cho từng mã (1 dòng/mã)
📜 CHÚ GIẢI: trích ngắn gọn chú giải chương/nhóm + SEN nếu có (3-5 dòng)
📋 KIỂM TRA CHUYÊN NGÀNH: giấy phép, kiểm dịch cần thiết (1-2 dòng, hoặc "Không yêu cầu")
🔍 TB-TCHQ: trích số TB nếu có (1-2 dòng), hoặc "✅ Không phát hiện tiền lệ"
⚠️ RỦI RO: mã dễ nhầm (1-2 dòng), hoặc "Rủi ro thấp"
🎯 KẾT LUẬN: mã đề xuất, điểm tự tin /100, lý do chọn, nguồn trích dẫn

QUAN TRỌNG: Viết NGẮN GỌN mỗi section. PHẢI hoàn thành TẤT CẢ 10 sections. Không viết dài dòng.`;
...
async function smartSearch(keywords, apiLog) {
  ...
  // For exact 8-digit codes: getHSDetail. For 4-digit chapters: getChapter → pick top items.
  let guessDetails = [];
  if (keywords.hs_guess?.length > 0) {
    const guessResults = await Promise.allSettled(
      keywords.hs_guess.slice(0, 4).map(async (code) => {
        ...
        // Exact match failed → search by chapter (first 4 digits)
        const chapter4 = code.substring(0, 4);
        apiLog.push({ step: 'search_t4_hs_guess', code, status: 'not_found_trying_chapter', chapter: chapter4 });
        try {
          const chapterData = await getChapter(chapter4.substring(0, 2));
          const items = (chapterData?.items || chapterData?.results || []);
          // Find items matching the 4-digit heading
          const matching = items.filter(i => {
            const hs = i.hs || i.ma_hs || i.hs_code || '';
            return hs.startsWith(chapter4);
          }).slice(0, 5);
          if (matching.length > 0) {
            apiLog.push({ step: 'search_t4_chapter_expand', chapter: chapter4, found: matching.length });
            // Fetch details for first 3 matching codes
            const expandResults = await Promise.allSettled(
              matching.slice(0, 3).map(async (item) => {
                const itemCode = item.hs || item.ma_hs || item.hs_code;
                const itemDetail = await getHSDetail(itemCode);
                return itemDetail?.found !== false ? { code: itemCode, ...itemDetail } : null;
              })
            );
        ...
        } catch { /* chapter search failed, continue */ }
        return null;
      })
    );
    for (const r of guessResults) {
      if (r.status === 'fulfilled' && r.value) {
        // Value can be a single detail or array of details (from chapter expand)
        const details = Array.isArray(r.value) ? r.value : [r.value];
        for (const d of details) {
          guessDetails.push(d);
          allResults.push({ hs: d.code, _source: 'hs_guess', vn: d.fact_layer?.vn || d.code });
        }
      }
    }
...
Bạn là chuyên gia phân loại HS Code Việt Nam. Phân tích mô tả hàng hóa và trích xuất từ khóa.

LƯU Ý: Người dùng có thể viết KHÔNG DẤU (VD: "xe dap" = "xe đạp", "cam bien nhiet" = "cảm biến nhiệt", "vit thep" = "vít thép"). Bạn PHẢI hiểu nghĩa và thêm DẤU đúng vào từ khóa.

CHỈ trả lời ĐÚNG JSON, KHÔNG có text nào khác trước hoặc sau JSON:
{
...
QUY TẮC:
- primary: 2-3 cụm từ kỹ thu��t chính xác CÓ DẤU trong biểu thuế VN (VD: "xe dap" → "xe đạp", "vit thep khong gi" → "vít thép không gỉ")
- short: 3-4 từ đơn ngắn gọn CÓ DẤU, dễ match — bao gồm tên phổ biến VÀ biến thể (VD: "xe đạp", "đạp", "bicycle")
- en: 2-3 tên tiếng Anh phổ biến (VD: "bicycle", "stainless steel screw", "temperature sensor")
- hs_guess: 2-4 mã HS 4-8 số — đoán RỘNG, bao gồm cả mã lân cận (VD: "8712", "87120030", "73181490")
- Ưu tiên đoán NHIỀU mã hơn là đoán CHÍNH XÁC — mục tiêu là không bỏ sót
- Nếu là ảnh: nhận diện sản phẩm rồi trích keywords
- KHÔNG viết text giải thích — CHỈ JSON`;
  ...
  try {
    ...
    let cleaned = keywordsRaw.replace(/```json\n?/g, '').replace(/```\n?/g, '').trim();
    // If LLM wrapped JSON in prose, extract the JSON object
  ...
  // Prioritize: hs_guess first (user-intent codes), then bieu_thue, tb_tchq
  const guessCodes = allResults
    .filter(r => r._source === 'hs_guess')
    .map(r => r.hs || r.ma_hs || r.hs_code)
    .filter(Boolean);
  const otherCodes = allResults
    .filter(r => r._source === 'bieu_thue' || r._source === 'tb_tchq')
    .slice(0, 3)
    .map(r => r.hs || r.ma_hs || r.hs_code)
    .filter(Boolean);
  const topCodes = [...new Set([...guessCodes, ...otherCodes])].slice(0, 5);
  ...
  // Token budget: ~4K tokens for context (Groq free tier = 6K TPM total)
  // Compact JSON (no pretty-print) + aggressive truncation
  const contextParts = [];

  if (allResults.length > 0) {
    const trimmedResults = allResults.slice(0, 8).map(r => ({
      hs: r.hs || r.ma_hs || r.hs_code,
      vn: (r.vn || r.ten_vn || r.mo_ta || '').substring(0, 80),
      src: r._source,
    }));
    contextParts.push(`TÌM KIẾM (${allResults.length} KQ, ${strategy}):${JSON.stringify(trimmedResults)}`);
  }
  if (searchSources.tb_tchq.length > 0) {
    const trimmedTB = searchSources.tb_tchq.slice(0, 3).map(t => ({
      tb: t.so_hieu, sp: (t.ten_san_pham || '').substring(0, 60), hs: t.ma_hs,
      ly_do: (t.ly_do_phan_loai || '').substring(0, 100),
    }));
    contextParts.push(`TB-TCHQ:${JSON.stringify(trimmedTB)}`);
  }
  if (searchSources.conflict.length > 0) {
    const trimmedConflict = searchSources.conflict.slice(0, 3).map(c => ({
      hs: c.hs || c.ma_hs, vn: (c.vn || c.mo_ta || '').substring(0, 60),
    }));
    contextParts.push(`CONFLICT:${JSON.stringify(trimmedConflict)}`);
  }
  if (hsDetails.length > 0) {
    // Compact 9 layers — only include non-null data, truncate text aggressively
    const trimmedDetails = hsDetails.slice(0, 3).map(d => {
      const detail = { code: d.code };
      // Tầng 1: Thuế suất (compact)
      if (d.fact_layer) {
        detail.tax = {
          vn: (d.fact_layer.vn || '').substring(0, 100),
      ...
      if (d.precedent_layer?.tb_tchq?.length) detail.tb = d.precedent_layer.tb_tchq.slice(0, 3).map(t => ({
        tb: t.so_hieu, sp: (t.ten_san_pham || '').substring(0, 50), hs: t.ma_hs,
      }));
      // Tầng 5-7: Compact
    ...
    contextParts.push(`9 TẦNG (${hsDetails.length} mã, top 3):${JSON.stringify(trimmedDetails)}`);
  }
