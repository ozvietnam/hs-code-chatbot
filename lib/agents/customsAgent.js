...
4. QUY TẮC BẤT DI BẤT DỊCH:
   ...
   - LUÔN HIỂN THỊ THUẾ dù confidence thấp hay cao, dù 1 mã hay nhiều mã ứng viên:
     Format bắt buộc: "Thuế NK (MFN): X% | ACFTA (TQ): X% | VAT: X%"
     Nếu field nào null/undefined → ghi "—" thay vì bỏ qua cả dòng
...
QUAN TRỌNG: Luôn kết thúc câu trả lời bằng dòng disclaimer sau (in nghiêng):
_⚠️ Thông tin trên chỉ mang tính chất tham khảo, không thay thế kết quả phân loại chính thức của cơ quan Hải quan. Vui lòng liên hệ Chi cục Hải quan hoặc Tổng cục Hải quan để được tư vấn chính thức._
`;
...
// ============================================================
// KB LEARNING — Save learning when user confirms (WS-001)
// ============================================================
/**
 * Save learning to knowledge base when user confirms an answer
 * Called after user thumbs up or confirms the HS code classification
 */
export async function saveLearningFromFeedback(productName, hsCode, confidence = 0.95) {
  if (!hsCode || !productName) return;

  try {
    const itemId = await addKnowledgeItem({
      type: 'hs_insight',
      content: `${productName} → HS ${hsCode} (từ user confirm)`,
      hsCodes: [hsCode.replace(/\./g, '')], // Remove dots for storage
      confidence: Math.min(confidence, 0.99), // Cap at 0.99 (user can still be wrong)
      source: 'user_feedback'
    });

    console.log(`[KB] Saved learning: ${productName} → ${hsCode} (ID: ${itemId})`);
    return itemId;
  } catch (err) {
    console.error('[KB] Error saving learning:', err.message);
    return null;
  }
}
...
export async function handleCustoms({ message, history, file, apiKey }) {
  ...
  // ═══════════════════════════════════════════════════════
  // CHU GIAI HANDLER — Xem chú giải chi tiết (follow-up shortcut)
  // ═══════════════════════════════════════════════════════
  if (isFollowUp === 'chu_giai') {
    const chuGiaiResult = await handleChuGiaiFollowUp(message, history);
    if (chuGiaiResult) return chuGiaiResult;
    // If no HS code in history or fetch failed, fall through to main agent
  }
  ...
  // ═══════════════════════════════════════════════════════
  // PHASE 1: VERDICT — Parse JSON with fallback strategies
  // ═══════════════════════════════════════════════════════
  let verdict;
  try {
    ...
    if (!cleaned.startsWith('{')) {
      ...
      if (match) {
        ...
        parseError = 'Could not find JSON object in response';
      }
    }

    // Strategy 3: Try to parse and handle errors
    try {
      verdict = JSON.parse(cleaned);
    } catch (e) {
      parseError = e.message;
      // Try to extract partial JSON (remove trailing comma, etc)
    ...
    if (!verdict || typeof verdict !== 'object') {
      throw new Error('Verdict is not a valid object');
    }

    const requiredFields = ['verdict', 'confidence', 'lookup_commands'];
    const missingFields = requiredFields.filter(f => !(f in verdict));
    if (missingFields.length > 0) {
      console.warn(`[VERDICT] Missing fields: ${missingFields.join(', ')}. Using defaults.`);
      // Fill missing fields with defaults
      if (!('verdict' in verdict)) verdict.verdict = message || '(unknown)';
      if (!('confidence' in verdict)) verdict.confidence = 50;
      if (!('lookup_commands' in verdict)) verdict.lookup_commands = [];
    }
    ...
    apiLog.push({
      ...
      parseError: parseError ? `(fixed: ${parseError})` : undefined,
    });
  } catch (err) {
    apiLog.push({
      step: 'verdict', status: 'error',
      error: err.message,
      detail: 'Fallback to keyword search'
    });
    ...
    verdict = {
      ...
      gir: 'GIR 1',
      lookup_commands: [
        { tool: 'search', params: text, why: 'tìm trực tiếp (fallback)' },
        ...(text.split(/\s+/).filter(w => w.length > 2).slice(0, 2).map(w => (
          { tool: 'search', params: w, why: 'từ khóa đơn (fallback)' }
        ))),
      ],
      what_could_change: '',
    };
...
// ============================================================
// CHU GIAI FOLLOW-UP HANDLER — trả chú giải từ data thực
// ============================================================
async function handleChuGiaiFollowUp(message, history) {
  const hsCode = extractHSCodeFromHistory(history);
  if (!hsCode) return null;

  let detail;
  try {
    detail = await getHSDetail(hsCode, 'legal_layer,fact_layer');
  } catch {
    return null;
  }
  if (!detail || detail.found === false) return null;

  const legal = detail.legal_layer;
  if (!legal) return null;

  const reply = formatChuGiaiResponse(hsCode, detail);
  return {
    reply,
...
function formatChuGiaiResponse(hsCode, detail) {
  const formattedHS = `${hsCode.slice(0, 4)}.${hsCode.slice(4, 6)}.${hsCode.slice(6, 8)}`;
  const legal = detail.legal_layer || {};
  ...
  parts.push(`📖 **CHÚ GIẢI** cho mã **${formattedHS}** — ${fact.vn || ''}`);

  if (legal.chu_giai_chuong) {
    const text = typeof legal.chu_giai_chuong === 'string'
      ? legal.chu_giai_chuong.substring(0, 1200)
      : JSON.stringify(legal.chu_giai_chuong).substring(0, 1200);
    parts.push(`\n**📋 Chú giải chương:**\n${text}`);
  }
  if (legal.chu_giai_phan) {
    const text = typeof legal.chu_giai_phan === 'string'
      ? legal.chu_giai_phan.substring(0, 500)
      : JSON.stringify(legal.chu_giai_phan).substring(0, 500);
    parts.push(`\n**📋 Chú giải phần:**\n${text}`);
  }
  if (legal.chu_giai_nhom) {
    const text = typeof legal.chu_giai_nhom === 'string'
      ? legal.chu_giai_nhom.substring(0, 600)
      : JSON.stringify(legal.chu_giai_nhom).substring(0, 600);
    parts.push(`\n**📋 Chú giải nhóm:**\n${text}`);
  }
  if (legal.bao_gom?.length > 0) {
    parts.push(`\n**✅ Bao gồm:**`);
    legal.bao_gom.slice(0, 6).forEach(i => parts.push(`- ${i}`));
    if (legal.bao_gom.length > 6) parts.push(`_...và ${legal.bao_gom.length - 6} mục khác_`);
  }
  if (legal.khong_bao_gom?.length > 0) {
    parts.push(`\n**❌ Không bao gồm:**`);
    legal.khong_bao_gom.slice(0, 6).forEach(i => parts.push(`- ${i}`));
    if (legal.khong_bao_gom.length > 6) parts.push(`_...và ${legal.khong_bao_gom.length - 6} mục khác_`);
  }
  if (legal.loai_tru?.length > 0) {
    parts.push(`\n**⛔ Loại trừ:**`);
    legal.loai_tru.slice(0, 4).forEach(i => parts.push(`- ${i}`));
  }
  if (legal.sen) {
    const senText = typeof legal.sen === 'string'
      ? legal.sen.substring(0, 600)
      : JSON.stringify(legal.sen).substring(0, 600);
    parts.push(`\n**📌 SEN AHTN 2022:**\n${senText}`);
  }
  if (legal.tinh_chat) {
    const tcText = typeof legal.tinh_chat === 'string'
      ? legal.tinh_chat.substring(0, 400)
      : JSON.stringify(legal.tinh_chat).substring(0, 400);
    parts.push(`\n**🔬 Tính chất:**\n${tcText}`);
  }

  // Tax rates
  const hasTax = rates.mfn || rates.acfta || rates.vat;
  if (hasTax) {
    parts.push(`\n**💰 Thuế suất (biểu thuế 2026):**`);
    parts.push(`- Thuế NK (MFN): **${rates.mfn ?? '—'}%**`);
    parts.push(`- ACFTA (TQ):     **${rates.acfta ?? '—'}%**`);
    if (rates.atiga) parts.push(`- ATIGA (ASEAN): **${rates.atiga}%**`);
    if (rates.cptpp) parts.push(`- CPTPP:         **${rates.cptpp}%**`);
    if (rates.evfta) parts.push(`- EVFTA:         **${rates.evfta}%**`);
    parts.push(`- VAT:           **${rates.vat ?? '—'}%**`);
  }

  parts.push(`\n💡 **Bạn có thể hỏi thêm:**`);
  parts.push(`*   **Kiểm tra chuyên ngành** — giấy phép, kiểm dịch, thủ tục`);
  parts.push(`*   **TB-TCHQ liên quan** — 4,390 tiền lệ phân loại`);
  parts.push(`*   **So sánh thuế FTA** — ACFTA/ATIGA/CPTPP/EVFTA`);
  parts.push(`*   **Mô tả khai ECUS** — gợi ý khai báo hải quan`);
