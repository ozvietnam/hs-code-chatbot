    ...
    if (fallbackChapters.length > 0) {
      const chapterResults = await Promise.allSettled(
        fallbackChapters.slice(0, 3).map(async (ch) => {
          apiLog.push({ step: 'fallback_chapter', code: `Ch.${ch}`, status: 'calling' });
          try {
            const data = await searchHS(ch.keyword, 15);
            const parsed = parseSearchResults(data);
            apiLog.push({ step: 'fallback_chapter', code: `Ch.${ch.chapter}`, status: 'done', resultCount: parsed.all.length });
            return parsed.all;
          } catch (e) {
            apiLog.push({ step: 'fallback_chapter', code: `Ch.${ch.chapter}`, status: 'error', error: e.message });
            return [];
          }
        })
      );
