...
async function executeInvestigation(plan, apiLog) {
  ...
  // FALLBACK: If investigation returned 0 results, use SHORT keywords from message
  if (allResults.length === 0) {
    apiLog.push({ step: 'investigate_fallback', status: 'calling' });

    // Build diverse short search terms from multiple sources
    const rawTerms = new Set();
    // From message: original words (2-word combos + single words)
    if (plan._originalMessage) {
      const words = plan._originalMessage.trim().split(/\s+/).filter(w => w.length > 1);
      // 2-word sliding window
      for (let i = 0; i < words.length - 1; i++) {
        rawTerms.add(words.slice(i, i + 2).join(' '));
      }
      // Single important words (>2 chars)
      words.filter(w => w.length > 2).forEach(w => rawTerms.add(w));
    }
    // From THINK: short fragments of product_understanding
    if (plan.product_understanding) {
      const puWords = plan.product_understanding.split(/[\s,()]+/).filter(w => w.length > 2 && !/^(là|và|có|của|này|được|trong|với|cho|một|không|từ|sản|phẩm)$/i.test(w));
      puWords.slice(0, 4).forEach(w => rawTerms.add(w));
      // 2-word combos from product understanding
      for (let i = 0; i < puWords.length - 1 && i < 3; i++) {
        rawTerms.add(puWords.slice(i, i + 2).join(' '));
      }
    }
    if (plan.material_function?.material && plan.material_function.material !== 'chưa rõ') {
      rawTerms.add(plan.material_function.material);
    }

    const fallbackTerms = [...rawTerms].slice(0, 6);
    apiLog.push({ step: 'investigate_fallback_terms', terms: fallbackTerms });
...
export async function handleCustoms({ message, history, file, apiKey }) {
  ...
  thinkResult._originalMessage = message;
